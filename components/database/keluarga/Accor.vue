...
const fetchVoters = async () => {
  try {
    ...
    const { data } = await useFetch('/api/votersKeluarga', {
      params: {
...
<template>
  <div class="p-4">
    ...
    <div v-else>
      ...
      <div class="overflow-x-auto rounded-lg border" v-for="(voter, index) in voters" :key="voter.nik">
        <div class="collapse bg-purple-300 mb-2">
            <input type="radio" name="my-accordion-1" checked />
            <div class="collapse-title text-xl font-medium">{{ voter.nama }}</div>
